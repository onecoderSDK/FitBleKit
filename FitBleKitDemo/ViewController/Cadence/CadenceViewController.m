...
@interface CadenceViewController ()<UITableViewDataSource, UITableViewDelegate, DeviceIdDelegate, FBKApiBsaeDataSource, FBKApiCadenceDelegate> {
...
@implementation CadenceViewController
...
- (void)viewDidLoad {
    ...
    for (int i = 0; i < 7; i++) {
        FBKApiCadence *cadenceApi = [[FBKApiCadence alloc] init];
        cadenceApi.delegate = self;
        cadenceApi.dataSource = self;
        cadenceApi.whellDiameter = 2.070;
        [cadenceApi setWhellDiameter:2.096];
        
        DeviceClass *myBleDevice = [[DeviceClass alloc] init];
        myBleDevice.bleDevice = cadenceApi;
...
- (void)backAction {
    ...
    for (int i = 0; i < m_deviceList.count; i++) {
        ...
        if (myBleDevice.deviceId.length != 0) {
            FBKApiCadence *cadenceApi = myBleDevice.bleDevice;
            [cadenceApi disconnectBleApi];
        }
    }
...
- (void)getDeviceId:(NSArray *)deviceIdArray {
    ...
    for (int i = 0; i < m_maxNumber; i++) {
        ...
        if (myBleDevice.isAvailable) {
            FBKApiCadence *cadenceApi = myBleDevice.bleDevice;
            [cadenceApi startConnectBleApi:myBleDevice.deviceId andIdType:DeviceIdUUID];
...
- (void)bleConnectStatus:(DeviceBleStatus)status andDevice:(id)bleDevice {
    ...
    for (int i = 0; i < m_deviceList.count; i++) {
        DeviceClass *myBleDevice = [m_deviceList objectAtIndex:i];
        FBKApiCadence *listApi = myBleDevice.bleDevice;
        if (cadenceApi == listApi) {
    ...
    DeviceClass *myBleDevice = [m_deviceList objectAtIndex:m_chooseNumber];
    FBKApiCadence *chooseApi = (FBKApiCadence *)myBleDevice.bleDevice;
    if (chooseApi == cadenceApi) {
        m_alertLab.text = [ShowTools showDeviceStatus:status];
        if (!cadenceApi.isConnected) {
            m_realTimeLab.text = @"speed (km/h) : --  \n\ncadence (times/min) : -- ";
        }
    }
}
...
- (void)devicePower:(NSString *)power andDevice:(id)bleDevice {
    ...
    if (chooseApi == cadenceApi) {
        NSString *bai = @"%";
        m_content.text = [NSString stringWithFormat:@"Device ID: %@\nPower: %@%@",cadenceApi.deviceId,power,bai];
    }
}
...
- (void)deviceFirmware:(NSString *)version andDevice:(id)bleDevice {
    ...
    FBKApiCadence *chooseApi = (FBKApiCadence *)myBleDevice.bleDevice;
    if (chooseApi == cadenceApi) {
        m_content.text = [NSString stringWithFormat:@"Device ID: %@\nFirmware version: %@",cadenceApi.deviceId,version];
    }
}
...
- (void)deviceHardware:(NSString *)version andDevice:(id)bleDevice {
    ...
    FBKApiCadence *chooseApi = (FBKApiCadence *)myBleDevice.bleDevice;
    if (chooseApi == cadenceApi) {
        m_content.text = [NSString stringWithFormat:@"Device ID: %@\nHardware version: %@",cadenceApi.deviceId,version];
    }
}
...
- (void)deviceSoftware:(NSString *)version andDevice:(id)bleDevice {
    ...
    FBKApiCadence *chooseApi = (FBKApiCadence *)myBleDevice.bleDevice;
    if (chooseApi == cadenceApi) {
        m_content.text = [NSString stringWithFormat:@"Device ID: %@\nSoftware version: %@",cadenceApi.deviceId,version];
    }
}
...
- (void)getCadence:(double)cadence andDevice:(id)bleDevice {
    m_realTimeLab.text = [NSString stringWithFormat:@"cadence (times/min) : %.2f ",cadence];
}
...
- (void)getSpeed:(double)speed withDistance:(double)distance andDevice:(id)bleDevice {
    m_realTimeLab.text = [NSString stringWithFormat:@"speed (m/s) : %.2f  \ndistance (m) : %.2f ",speed,distance];
}
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if (tableView == m_deviceTableView) {
        ...
        DeviceClass *myBleDevice = [m_deviceList objectAtIndex:m_chooseNumber];
        FBKApiCadence *cadenceApi = (FBKApiCadence *)myBleDevice.bleDevice;
        if (cadenceApi.isConnected) {
            if (myRow == 0) {
                [cadenceApi readDevicePower];
            }
            else if (myRow == 1) {
                [cadenceApi readFirmwareVersion];
            }
            else if (myRow == 2) {
                [cadenceApi readHardwareVersion];
            }
            else if (myRow == 3) {
                [cadenceApi readSoftwareVersion];
            }
            else if (myRow == 4)
            {
                [cadenceApi getPrivateMacAddress];
            }
            else if (myRow == 5)
            {
                [cadenceApi getPrivateVersion];
            }
            else if (myRow == 6)
            {
                [cadenceApi enterOTAMode];
            }
        }
    }
}
