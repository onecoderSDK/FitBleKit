...
@interface RunningViewController ()<UITableViewDataSource, UITableViewDelegate, DeviceIdDelegate, FBKApiBsaeDataSource, FBKApiRunDelegate> {
...
- (void)viewDidLoad {
    ...
    for (int i = 0; i < 7; i++) {
        FBKApiRunning *runApi = [[FBKApiRunning alloc] init];
        runApi.delegate = self;
        runApi.dataSource = self;
        
        DeviceClass *myBleDevice = [[DeviceClass alloc] init];
        myBleDevice.bleDevice = runApi;
...
- (void)backAction {
    ...
    for (int i = 0; i < m_deviceList.count; i++) {
        ...
        if (myBleDevice.deviceId.length != 0) {
            FBKApiRunning *runApi = myBleDevice.bleDevice;
            [runApi disconnectBleApi];
        }
    }
...
- (void)getDeviceId:(NSArray *)deviceIdArray {
    ...
    for (int i = 0; i < m_maxNumber; i++) {
        ...
        if (myBleDevice.isAvailable) {
            FBKApiRunning *runApi = myBleDevice.bleDevice;
            [runApi startConnectBleApi:myBleDevice.deviceId andIdType:DeviceIdUUID];
...
- (void)bleConnectStatus:(DeviceBleStatus)status andDevice:(id)bleDevice {
    FBKApiRunning *runApi = (FBKApiRunning *)bleDevice;
    for (int i = 0; i < m_deviceList.count; i++) {
        DeviceClass *myBleDevice = [m_deviceList objectAtIndex:i];
        FBKApiRunning *listApi = myBleDevice.bleDevice;
        if (runApi == listApi) {
    ...
    DeviceClass *myBleDevice = [m_deviceList objectAtIndex:m_chooseNumber];
    FBKApiRunning *chooseApi = (FBKApiRunning *)myBleDevice.bleDevice;
    if (chooseApi == runApi) {
...
- (void)devicePower:(NSString *)power andDevice:(id)bleDevice {
    ...
    if (chooseApi == runApi) {
        NSString *bai = @"%";
        m_content.text = [NSString stringWithFormat:@"Device ID: %@\nPower: %@%@",runApi.deviceId,power,bai];
    }
}
...
- (void)deviceFirmware:(NSString *)version andDevice:(id)bleDevice {
    ...
    FBKApiRunning *chooseApi = (FBKApiRunning *)myBleDevice.bleDevice;
    if (chooseApi == runApi) {
        m_content.text = [NSString stringWithFormat:@"Device ID: %@\nFirmware version: %@",runApi.deviceId,version];
    }
}
...
- (void)deviceHardware:(NSString *)version andDevice:(id)bleDevice {
    ...
    FBKApiRunning *chooseApi = (FBKApiRunning *)myBleDevice.bleDevice;
    if (chooseApi == runApi) {
        m_content.text = [NSString stringWithFormat:@"Device ID: %@\nHardware version: %@",runApi.deviceId,version];
    }
}
...
- (void)deviceSoftware:(NSString *)version andDevice:(id)bleDevice {
    ...
    FBKApiRunning *chooseApi = (FBKApiRunning *)myBleDevice.bleDevice;
    if (chooseApi == runApi) {
        m_content.text = [NSString stringWithFormat:@"Device ID: %@\nSoftware version: %@",runApi.deviceId,version];
    }
}
...
- (void)realTimeRunning:(NSDictionary *)runMap andDevice:(id)bleDevice {
    ...
    FBKApiRunning *chooseApi = (FBKApiRunning *)myBleDevice.bleDevice;
    if (chooseApi == runApi) {
        m_content.text = [NSString stringWithFormat:@"Device ID: %@\n\nRunning Data: %@",runApi.deviceId,runMap];
    }
}
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if (tableView == m_deviceTableView) {
        ...
        DeviceClass *myBleDevice = [m_deviceList objectAtIndex:m_chooseNumber];
        FBKApiRunning *runApi = (FBKApiRunning *)myBleDevice.bleDevice;
        if (runApi.isConnected) {
            if (myRow == 0) {
                [runApi readDevicePower];
            }
            else if (myRow == 1) {
                [runApi readFirmwareVersion];
            }
            else if (myRow == 2) {
                [runApi readHardwareVersion];
            }
            else if (myRow == 3) {
                [runApi readSoftwareVersion];
            }
            else if (myRow == 4)
            {
                [runApi getPrivateMacAddress];
            }
            else if (myRow == 5)
            {
                [runApi getPrivateVersion];
            }
            else if (myRow == 6)
            {
                [runApi enterOTAMode];
