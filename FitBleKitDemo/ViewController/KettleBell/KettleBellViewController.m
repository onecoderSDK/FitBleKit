/********************************************************************************
 * 文件名称：KettleBellViewController.m
 * 内容摘要：壶铃
 * 版本编号：1.0.1
 * 创建日期：2018年04月02日
 ********************************************************************************/
...
@interface KettleBellViewController ()<UITableViewDataSource, UITableViewDelegate, DeviceIdDelegate, FBKApiBsaeDataSource, FBKApiKettleBellDelegate> {
...
@implementation KettleBellViewController
...
- (void)viewDidLoad {
    ...
    for (int i = 0; i < 7; i++) {
        FBKApiKettleBell *kettleBellApi = [[FBKApiKettleBell alloc] init];
        kettleBellApi.delegate = self;
        kettleBellApi.dataSource = self;
        
        DeviceClass *myBleDevice = [[DeviceClass alloc] init];
        myBleDevice.bleDevice = kettleBellApi;
...
- (void)backAction {
    ...
    for (int i = 0; i < m_deviceList.count; i++) {
        ...
        if (myBleDevice.deviceId.length != 0) {
            FBKApiKettleBell *kettleBellApi = myBleDevice.bleDevice;
            [kettleBellApi disconnectBleApi];
        }
    }
...
- (void)chooseDevice {
    ...
    if (myBleDevice.deviceId.length > 0) {
        ...
        bindView.scanDeviceType = BleDeviceKettleBell;
...
- (void)getDeviceId:(NSArray *)deviceIdArray {
    ...
    for (int i = 0; i < m_maxNumber; i++) {
        ...
        if (myBleDevice.isAvailable) {
            FBKApiKettleBell *kettleBellApi = myBleDevice.bleDevice;
            [kettleBellApi startConnectBleApi:myBleDevice.deviceId andIdType:DeviceIdUUID];
...
- (void)bleConnectStatus:(DeviceBleStatus)status andDevice:(id)bleDevice {
    FBKApiKettleBell *kettleBellApi = (FBKApiKettleBell *)bleDevice;
    for (int i = 0; i < m_deviceList.count; i++) {
        DeviceClass *myBleDevice = [m_deviceList objectAtIndex:i];
        FBKApiKettleBell *listApi = myBleDevice.bleDevice;
        if (kettleBellApi == listApi) {
    ...
    DeviceClass *myBleDevice = [m_deviceList objectAtIndex:m_chooseNumber];
    FBKApiKettleBell *chooseApi = myBleDevice.bleDevice;
    if (chooseApi == kettleBellApi) {
...
- (void)devicePower:(NSString *)power andDevice:(id)bleDevice {
    ...
    DeviceClass *myBleDevice = [m_deviceList objectAtIndex:m_chooseNumber];
    FBKApiKettleBell *chooseApi = myBleDevice.bleDevice;
    if (kettleBellApi == chooseApi) {
        NSString *bai = @"%";
        m_content.text = [NSString stringWithFormat:@"Device ID: %@\nPower: %@%@",kettleBellApi.deviceId,power,bai];
    }
}
...
- (void)deviceFirmware:(NSString *)version andDevice:(id)bleDevice {
    ...
    FBKApiKettleBell *chooseApi = myBleDevice.bleDevice;
    if (kettleBellApi == chooseApi) {
        m_content.text = [NSString stringWithFormat:@"Device ID: %@\nFirmware version: %@",kettleBellApi.deviceId,version];
    }
}
...
- (void)deviceHardware:(NSString *)version andDevice:(id)bleDevice {
    ...
    FBKApiKettleBell *chooseApi = myBleDevice.bleDevice;
    if (kettleBellApi == chooseApi) {
        m_content.text = [NSString stringWithFormat:@"Device ID: %@\nHardware version: %@",kettleBellApi.deviceId,version];
    }
}
...
- (void)deviceSoftware:(NSString *)version andDevice:(id)bleDevice {
    ...
    FBKApiKettleBell *chooseApi = myBleDevice.bleDevice;
    if (kettleBellApi == chooseApi) {
        m_content.text = [NSString stringWithFormat:@"Device ID: %@\nSoftware version: %@",kettleBellApi.deviceId,version];
    }
}
...
- (void)kettleBellVersion:(NSString *)version andDevice:(id)bleDevice {
    ...
    FBKApiKettleBell *chooseApi = myBleDevice.bleDevice;
    if (kettleBellApi == chooseApi) {
        m_content.text = [NSString stringWithFormat:@"Device ID: %@\nProtocol version: %@",kettleBellApi.deviceId,version];
    }
}


/********************************************************************************
 * 方法名称：kettleBellDetailData
 * 功能描述：大数据
 * 输入参数：
 * 返回数据：
 ********************************************************************************/
- (void)kettleBellDetailData:(NSDictionary *)dataDic andDevice:(id)bleDevice {
    ...
    if (kettleBellApi == chooseApi) {
        ...
        NSArray *recordArray = [dataDictionary objectForKey:@"kettleBellData"];
        if (recordArray.count > 0) {
            BellListViewController *listView = [[BellListViewController alloc] init];
            listView.dataArray = recordArray;
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    static NSString *cellString = @"KettleBellCell";
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if (tableView == m_deviceTableView) {
        ...
        DeviceClass *myBleDevice = [m_deviceList objectAtIndex:m_chooseNumber];
        FBKApiKettleBell *kettleBellApi = (FBKApiKettleBell *)myBleDevice.bleDevice;
        if (kettleBellApi.isConnected) {
            if (myRow == 0) {
                [kettleBellApi getKettleBellTotalHistory];
            }
            else if (myRow == 1) {
                [kettleBellApi readDevicePower];
            }
            else if (myRow == 2) {
                [kettleBellApi readFirmwareVersion];
            }
            else if (myRow == 3) {
                [kettleBellApi readHardwareVersion];
            }
            else if (myRow == 4) {
                [kettleBellApi readSoftwareVersion];
            }
            else if (myRow == 5)
            {
                [kettleBellApi getPrivateMacAddress];
            }
            else if (myRow == 6)
            {
                [kettleBellApi getPrivateVersion];
            }
            else if (myRow == 7)
            {
                [kettleBellApi enterOTAMode];
            }
        }
    }
}
