/********************************************************************************
 * 文件名称：ErgometerViewController.m
 * 内容摘要：拉力计
 * 版本编号：1.0.1
 * 创建日期：2023年02月28日
 ********************************************************************************/
...
- (void)viewDidLoad {
    ...
    for (int i = 0; i < 7; i++) {
        FBKApiErgometer *ergApi = [[FBKApiErgometer alloc] init];
        ergApi.delegate = self;
        ergApi.dataSource = self;
        
        DeviceClass *myBleDevice = [[DeviceClass alloc] init];
        myBleDevice.bleDevice = ergApi;
...
- (void)getDeviceId:(NSArray *)deviceIdArray {
    ...
    for (int i = 0; i < m_maxNumber; i++) {
        ...
        if (myBleDevice.isAvailable) {
            FBKApiErgometer *ergApi = myBleDevice.bleDevice;
            [ergApi startConnectBleApi:myBleDevice.deviceId andIdType:DeviceIdUUID];
...
- (void)bleConnectStatus:(DeviceBleStatus)status andDevice:(id)bleDevice {
    ...
    for (int i = 0; i < m_deviceList.count; i++) {
        DeviceClass *myBleDevice = [m_deviceList objectAtIndex:i];
        FBKApiErgometer *listApi = myBleDevice.bleDevice;
        if (ergApi == listApi) {
...
- (void)devicePower:(NSString *)power andDevice:(id)bleDevice {
    ...
    if (chooseApi == ergApi) {
        NSString *bai = @"%";
        m_content.text = [NSString stringWithFormat:@"Device ID: %@\nPower: %@%@",ergApi.deviceId,power,bai];
    }
}
...
- (void)deviceFirmware:(NSString *)version andDevice:(id)bleDevice {
    ...
    FBKApiErgometer *chooseApi = (FBKApiErgometer *)myBleDevice.bleDevice;
    if (chooseApi == ergApi) {
        m_content.text = [NSString stringWithFormat:@"Device ID: %@\nFirmware version: %@",ergApi.deviceId,version];
    }
}
...
- (void)deviceHardware:(NSString *)version andDevice:(id)bleDevice {
    ...
    FBKApiErgometer *chooseApi = (FBKApiErgometer *)myBleDevice.bleDevice;
    if (chooseApi == ergApi) {
        m_content.text = [NSString stringWithFormat:@"Device ID: %@\nHardware version: %@",ergApi.deviceId,version];
    }
}
...
- (void)deviceSoftware:(NSString *)version andDevice:(id)bleDevice {
    ...
    FBKApiErgometer *chooseApi = (FBKApiErgometer *)myBleDevice.bleDevice;
    if (chooseApi == ergApi) {
        m_content.text = [NSString stringWithFormat:@"Device ID: %@\nSoftware version: %@",ergApi.deviceId,version];
    }
}
